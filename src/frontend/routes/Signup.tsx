...
const Signup: React.FC = () => { 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await setDoc(doc(db, "users", userCred.user.uid), { email, judet }, { merge: true }); // Optionally merge judet
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      const role = await createUserWithRole(result.user.uid, result.user.email || "");
      await setDoc(doc(db, "users", result.user.uid), { email: result.user.email || "", judet }, { merge: true });
      setSuccess(`Signed up with Google as ${role}!`);
    } catch (err) {
      const errorMsg = (err as { message?: string }).message || "Google sign up failed";
      setError(errorMsg);
    } finally {
      setLoading(false);
    }
      ...
      <form className="w-full flex flex-col gap-4" onSubmit={handleSubmit}>
        ...
        <div className="flex justify-center mt-2">
          <button
            ...
            className="flex items-center bg-white border border-google-button-border-light rounded-md p-0.5 pr-3"
